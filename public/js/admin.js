...
function setupSettingsForm() {
  ...
  if (urlInput) urlInput.value = localStorage.getItem("KDS_SUPABASE_URL") || window.KDS.ACTIVE_SUPABASE_URL || "";
  if (keyInput) keyInput.value = localStorage.getItem("KDS_SUPABASE_ANON_KEY") || window.KDS.ACTIVE_SUPABASE_ANON_KEY || "";
    ...
    btnSaveConfig.addEventListener("click", () => {
      ...
      if (newUrl) {
        if (!newUrl.startsWith("https://")) {
          showToast("Invalid Supabase URL: URL must start with https:// (e.g. https://xyzcompany.supabase.co)", "error");
          return;
        }
      }
      ...
      if (newUrl) localStorage.setItem("KDS_SUPABASE_URL", newUrl);
      else localStorage.removeItem("KDS_SUPABASE_URL");

      if (newKey) localStorage.setItem("KDS_SUPABASE_ANON_KEY", newKey);
      else localStorage.removeItem("KDS_SUPABASE_ANON_KEY");
  ...
  if (btnDisconnect) {
    btnDisconnect.addEventListener("click", () => {
      if (confirm("Disconnect Supabase? The application will fall back to local storage and offline data.")) {
        localStorage.removeItem("KDS_SUPABASE_URL");
        localStorage.removeItem("KDS_SUPABASE_ANON_KEY");
        if (urlInput) urlInput.value = "";
        if (keyInput) keyInput.value = "";
        showToast("Supabase disconnected. Reloading to local mode...", "success");
        setTimeout(() => {
          window.location.reload();
        }, 1000);
      }
  ...
  if (btnTestConn) {
    btnTestConn.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast("Supabase Connection Successful! Database table 'site_data' is accessible.", "success");
      } catch (connErr) {
